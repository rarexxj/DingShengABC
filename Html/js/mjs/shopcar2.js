        ...
        methods: {
            infoajax: function () {
                ...
                $.ajax({
                    ...
                    if (rs.returnCode == '200') {
                        _this.info = rs.data;
                        console.log(rs.data.InvestmentGoods.length)
                        if(rs.data.InvestmentGoods.length==0){
                            $('.nothing').show();
                            $('.infoList').hide();
                        }
                    }
            ...
            price: function () {
                ...
                $('.quan').each(function () {
                    ...
                    console.log(_price);
                    var _num=Number($(this).siblings('a').find('.num').val());
                    console.log(_num)
                    if ($(this).hasClass('active')) {
                        sumprice = sumprice + _price*_num;
                        btn = true;
                    }
                    if (!btn) {
                        _price = 0
            ...
            link: function () {
                ...
                if ($('.quan.active').length == 0) {
                    ...
                    window.location.href = "/Html/getmethodgwc.html?id=" + idstr + '&type=' + 1
                    // window.location.href = "/Html/settlement.html?id=" + idstr
                }
            },
            linkaddress:function () {
                //冒泡
                $("#main").on("click",'.linkaddress', function (e) {
                    var evt = e || window.event;
//              evt.stopPropagation();
                    if($(evt.target).is('.reduce')||$(evt.target).is('.add')||$(evt.target).is('.num')){
                        evt.preventDefault();
