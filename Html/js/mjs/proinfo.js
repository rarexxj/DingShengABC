...
$(function () {
    ...
    function ajaxeva(callback, beforecallback) {
        $.ajax({
            ...
            if (rs.returnCode == '200') {
                // console.log(1);
                $('.pingjianum').html(rs.data.TotalCount);
                vieweva(rs.data);
    ...
    function vieweva(rs) {
        ...
        for (i in rs.Evaluates) {
            rs.Evaluates[i].newtime = rs.Evaluates[i].EvaluateTime.split(' ')[0]
        }

        for (i in rs.Evaluates) {
            if (rs.Evaluates[i].ReplyTime) {
                rs.Evaluates[i].newrtime = rs.Evaluates[i].ReplyTime.split(' ')[0]
            }
        }
        if (eval) {
            ...
            eval = new Vue({
                ...
                data: rs,
                ready: function () {
                    // console.log($('.content').attr('data-pingjinum'))
                    $('.pingjianum').html()
    ...
    //立即购买
    $('.infoBox').on('click', '.pro-in-gobuy', function () {
        if (!$('.get-btn').attr('data-id')) {
            oppo('请选择规格',1);
            // $('.size-mask').show();
        } else {
            if ($('.kc').html() == 0) {
                ...
                window.location.href = "/Html/ShopCar/Confirm.html?gid=" + $('.get-btn').attr('data-id') + "|" + $('.getnum .amount').val() + "&type=" + 0
            }
