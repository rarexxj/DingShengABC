var mainTreeFun = {
    ...
    onClick: function (e, treeId, treeNode) {
        ...
        for (var i = 0, l = nodes.length; i < l; i++) {
            v += nodes[i].name + ",";
            cid += nodes[i].id + ",";
            valuationType += nodes[i].ValuationType + ",";
            goodsTypeId += nodes[i].GoodsTypeId + ",";
        }
        if (v.length > 0) v = v.substring(0, v.length - 1);
        if (cid.length > 0) cid = cid.substring(0, cid.length - 1);
        if (valuationType.length > 0) valuationType = valuationType.substring(0, valuationType.length - 1);
        if (goodsTypeId.length > 0) goodsTypeId = goodsTypeId.substring(0, goodsTypeId.length - 1);
        $("#CategoryName").attr("value", v);
        $("#CategoryId").attr("value", cid);
        $("#ValuationType").attr("value", valuationType);
        $("#GoodsTypeId").attr("value", goodsTypeId);
        $("#Good_Bnt").click();
