...
jQuery(function ($) {
    ...
    $("#AssemblyList").on("change", ".processingFee", function () {
        var singleProductId = $(this).data("singe-product-id");
        var index = _.findIndex(singleGoodsStore, function (o) { return o.Id === singleProductId; });
        var processingFee = $(this).val();
        singleGoodsStore[index].ProcessingFee = _.toNumber(processingFee);

       
    });
    $("#AssemblyList").on("change", ".deposit", function () {
        var singleProductId = $(this).data("singe-product-id");
        var index = _.findIndex(singleGoodsStore, function (o) { return o.Id === singleProductId; });
        var deposit = $(this).val();
        singleGoodsStore[index].Deposit = _.toNumber(deposit);
    ...
    $("#AssemblyList").on("change", ".price", function () {
        ...
        //取最小值显示到主表
        //var min = _.minBy(singleGoodsStore, function (o) { return o.ProcessingFee; });
        //$("#ShopPrice").val(min.ProcessingFee);
    });
...
function eachAttrVals(surplusAttrs, joinVals) {
    ...
    if (surplusAttrs.length === 0) {
        ...
        if (!_.isUndefined(currentGoods)) {
            ...
            deposit = currentGoods.Deposit;
            processingFee = currentGoods.ProcessingFee;
            valuationType = $("#ValuationType").val();
        }
        var singleGoods = { Id: singleGoodsId, Stock: stock, Unit: unit, Price: price,Deposit:deposit, Attrs: joinVals, Image: image, ProcessingFee: processingFee, ValuationType: valuationType };
        ...
        render += '<div class="clearfix"><span class="profile-picture pull-left"><img data-singe-product-id="' + singleGoodsId + '" class="editable img-responsive" src="' + imageUrl + '" /></span><div class="pull-left singleGoodsInfo"><div>' + title + '</div><div>库存：<input class="stock" data-singe-product-id="' + singleGoodsId + '" style="width: 100px;" value="' + stock + '"/> 单位：<select class="unit" style="width: 50px;" data-singe-product-id="' + singleGoodsId + '" >' + options;
        if ($("#GoodsTypeId").val() == "44fbc7b6-a414-4c84-a74d-daf3ad6857e1") {
            if ($("#ValuationType").val()=="1") {
                render += '</select> 价格：<input class="price" style="width: 100px;" data-singe-product-id="' + singleGoodsId + '" value="' + price + '"/>';
            }
            render += '</select> 定金：<input class="deposit" style="width: 100px;" data-singe-product-id="' + singleGoodsId + '" value="' + deposit + '"/>';
        }
        if ($("#GoodsTypeId").val() == "9bc410e7-b955-4546-812c-cd9a21bb0b9a") {
            if ($("#ValuationType").val() == "1") {
                render += '</select> 价格：<input class="price" style="width: 100px;" data-singe-product-id="' + singleGoodsId + '" value="' + price + '"/>';
            } else {
                render += '</select> 工艺费：<input class="processingFee" style="width: 100px;" data-singe-product-id="' + singleGoodsId + '" value="' + processingFee + '"/>';
            }
        }

    render += '</div></div></div><hr />';
